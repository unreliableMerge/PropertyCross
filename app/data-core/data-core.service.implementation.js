...
var dataCoreService = function ($http) {
    var searchingData = {
        responsedData: [],
        commonPageInformation: {}
    };

    var NESTORIA_API_URL = 'http://api.nestoria.co.uk/api?country=uk&pretty=1&action=search_listings&encoding=json&listing_type=buy&callback=JSON_CALLBACK';
    var PAGE_NUMBER = '&page=';
    var PLACE_NAME = '&place_name=';
    var CENTRE_POINT = '&centre_point=';
        ...
        return $http.jsonp(NESTORIA_API_URL +
            ...
            .then(function (response) {
                ...
                if (searchingData.responsedData.length > 0) {
                    dataCoreStoringService.addInputRequest(requestName,
    ...
    var _responsedData = function () {
        var takeFromLocaleStorage = dataCoreStoringService.readInputRequest();
        if (searchingData.responsedData.length == 0) {
            return $http.jsonp(NESTORIA_API_URL +
